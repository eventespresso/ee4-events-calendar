...
{
    protected string $_new_meta_table;
    ...
    {
        $term_id =
            EE_Data_Migration_Manager::instance()->get_mapping_new_pk(
                'Core_4_1_0',
                $this->_old_table,
                $old_row['id'],
                $this->_new_table
        ...
        if (! $term_id) {
            $this->add_error(
                sprintf(
                    __(
                        "Could not migrate metadata for old category '%s' with id '%d' because there is no EE4 category (term) for it.",
                        "event_espresso"
                    ),
                    $old_row['category_name'],
                    $old_row['id']
                )
            );
            return;
